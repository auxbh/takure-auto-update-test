...
#[cfg(target_arch = "x86")]
const RELEASE_ASSET_NAME: &str = "takure_32.zip";

#[cfg(target_arch = "x86_64")]
const RELEASE_ASSET_NAME: &str = "takure_64.zip";
...
struct UpdateInformation {
    pub version: String,
    pub commit: String,
    pub sha256_32: String,
    pub sha256_64: String,
}

#[cfg(target_arch = "x86")]
fn expected_sha256(info: &UpdateInformation) -> &str {
    &info.sha256_32
}
...
pub fn self_update(module: &LibraryHandle) -> Result<bool, SelfUpdateError> {
    ...
    let url = format!(
        ...
        UPDATE_REPO, response.version, RELEASE_ASSET_NAME
    );
    ...
    debug!("Extracting takure.dll from downloaded archive...");
    let new_hook = {
        let mut archive =
            zip::ZipArchive::new(Cursor::new(archive_bytes)).context(InvalidArchiveSnafu)?;
        let mut file = archive
            .by_name("takure.dll")
            .map_err(|_| SelfUpdateError::MissingDllInArchive)?;
        let mut buf = Vec::with_capacity(file.size() as usize);

        file.read_to_end(&mut buf).context(FailedDownloadingUpdateSnafu)?;
